...
## One-paste spec for the Trick-task board

This blueprint calibrates the Trick-task board on a PeakWatch Ultra triage bot that routes warranty and billing tickets to the right queue.

---

## Specimen under inspection

**Bot:** PeakWatch Ultra triage bot that routes warranty and billing tickets to the right queue
...
**Usage reality:** Zendesk export shows mixed warranty and billing asks with product names dominating the subject line every Tuesday

**Source:** Zendesk PeakWatch queue export, week of 7 Aug 2026

---

## Raw messages for trick tasks
...
## Eight trick tasks with verdicts and defenses

| Task ID | Verdict | Finding | Defense to flip |
|---------|---------|---------|-----------------|
...
## Defense settings (four required)

1. **current-policy fetch before answer** — ensures stale policy references are replaced with live data
2. **ask-type before product match** — routes on customer intent, not product name in subject
3. **billing intent watch before product nouns** — catches billing asks that mention product names
4. **blocking invariant until aligned** — hard-blocks ship when terminology drift is detected

---

## Block number

**2** — two or more failures on default settings block ship

---

## Hard block condition

Silent-drift divergence (p6) is a hard block regardless of other pass/fail counts.

---

## Attach rule

The full board must be attached to any proposal that reaches Marisol — a proposal without the board does not count as submitted.
...
This blueprint is calibrated on the PeakWatch Ultra triage bot. The builder's own board — eight verdicts, the defenses they turned on, the rule they wrote and the failure they named — is embedded as the worked example, so a stranger gets the same discipline applied to their own bot.
